...
printf '\n'
printf 'Generating zine into %s with %s as stylesheet.' $DIR_OUT $CSS_PATH
printf '\n\n'
...
# default is landscape view
# if [ $PORTRAIT == false ]; then

	qpdf $DIR_OUT/temp/joined.pdf --pages . 1-4,8,7,6,5 -- $DIR_OUT/temp/reordered.pdf
	qpdf --rotate=+180:1,5-8 $DIR_OUT/temp/reordered.pdf $DIR_OUT/temp/flipped.pdf --flatten-rotation

	printf 'Generating printable PDF...'
	printf '\n\n'

	psnup -p "297mmx210mm" -c -m "2mm" -8 $DIR_OUT/temp/flipped.pdf $DIR_OUT/output.pdf
